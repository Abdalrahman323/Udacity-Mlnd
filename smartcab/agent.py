...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=True, epsilon=1.0, alpha=0.5):
        ...
        self.tmp = dict((k, 0.0) for k in self.valid_actions)
        #print(self.tmp)

        for state_tuple in itertools.product(*self.state_features):
            #print(state_tuple)
            #print(self.tmp)
            self.Q[state_tuple] = self.tmp.copy()
    ...
    def choose_action(self, state):
        ...
        if(self.learning == False or random.random() <= self.epsilon):
            ...
            #print(action)
        else: # choosing an action with the highest Q-value
            valid_actions = []
            maxQ = self.get_maxQ(state)
            for tmp_action in self.Q[state]:
                if maxQ == self.Q[state][tmp_action]:
                    valid_actions.append(tmp_action)
